...
async def process_message(msg, session):
    ...
    try:
        ...
        create_invoice_task = asyncio.create_task(generate_tx, pubkey, mint, amount, slippage, priorityFee)
        checker_task = asyncio.create_task(checker(session, uri, creator))
