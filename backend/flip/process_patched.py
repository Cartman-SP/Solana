...
# ==== Commission aggregation helper (base + priority + small native tips) ====
TIP_MAX_LAMPORTS = 1_000_000_000  # 0.01 SOL

def sum_all_fees_helius(tx: dict) -> int:
    """
    Возвращает сумму всех КОМИССИЙ по транзакции в лампортах:
      - базовая fee
      - приоритетная (priorityFee или computeUnitsConsumed * computeUnitsPriceMicroLamports)
      - Jito/роутер чаевые: Небольшие nativeTransfers ИЗ feePayer, каждый <= 0.01 SOL
    ВНИМАНИЕ: большие переводы SOL на пулы/программы считаются оплатой свопа, а не комиссией.
    """
    # 1) базовый газ
    try:
    ...
    # 2) приоритетная часть
    priority_fee = 0
    try:
        if tx.get("priorityFee") is not None:
            priority_fee = int(tx.get("priorityFee") or 0)
        else:
            cu = tx.get("computeUnitsConsumed")
            price_micro = tx.get("computeUnitsPriceMicroLamports")
            if cu and price_micro:
                priority_fee = (int(cu) * int(price_micro)) // 1_000_000
    except Exception:
        priority_fee = 0

    # 3) чаевые/роутерные сборы (малые нативные переводы из feePayer)
    tips = 0
    try:
        ...
        for nt in (tx.get("nativeTransfers") or []):
            if nt.get("fromUserAccount") == fee_payer:
                ...
                if amt <= TIP_MAX_LAMPORTS:
                    tips += amt
    except Exception:
        pass

    return int(base_fee + priority_fee + tips)
...
async def get_values_total_and_fees_helius(
    ...
    for _ in range(5):
        ...
        for tx in page:
            ...
            if change != 0.0:
                values.append(change)
                        # суммируем комиссии (base + priority + small native tips)
            try:
                total_fees += float(sum_all_fees_helius(tx))
            except Exception:
                pass
